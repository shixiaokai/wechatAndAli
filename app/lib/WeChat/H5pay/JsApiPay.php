<?php
namespace App\lib\WeChat\H5pay;
...
class JsApiPay
{
    ...
    public function GetOpenid()
    {
        ...
        if (!(isset($_GET['code']) && $_GET['code'])){
            ...
            $baseUrl = urlencode($baseUrl.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
    ...
    public function GetOpenidFromMp($code)
    {
        ...
        $openid = $data['openid'];
        //return $data;
        //$openid = isset($data['openid'])?$data['openid']:false;
        return $openid;
    }
    ...
    {
        $urlObj["appid"] = env('WX_APPID');
        $urlObj["secret"] = env('WX_APPSECRET');
        $urlObj["code"] = $code;
    ...
    {
        //通过code获得openid
        if (!isset($_GET['code']) || (isset($_GET['code']) && Redis::sismember('wechat_code',$_GET['code'])))
        {
            //触发微信返回code码
            $baseUrl = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://";
            $baseUrl = urlencode($baseUrl.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
